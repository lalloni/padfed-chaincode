...
func (s *SmartContract) savePersona(APIstub shim.ChaincodeStubInterface, cuit uint64, newPersona *Persona) Response {
	...
	case "J":
		...
		if !(newPersona.IDFormaJuridica >= 1 && newPersona.IDFormaJuridica <= 999) {
			return clientErrorResponse("formaJuridica [" + strconv.Itoa(int(newPersona.IDFormaJuridica)) + "] invalida")
		}
	...
	case "":
		personaNull = true
		if len(newPersona.RazonSocial) > 0 {
			return clientErrorResponse("razonSocial debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.Nombre) > 0 {
			return clientErrorResponse("nombre debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.EstadoCUIT) > 0 {
			return clientErrorResponse("estado debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.Apellido) > 0 {
			return clientErrorResponse("apellido debe ser nulo cuando persona es nulo")
		}
		if newPersona.IDFormaJuridica != 0 {
			return clientErrorResponse("formaJuridica debe ser nulo cuando persona es nulo")
		}
		if newPersona.TipoDoc != 0 {
			return clientErrorResponse("tipoDoc debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.Documento) > 0 {
			return clientErrorResponse("doc debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.Sexo) > 0 {
			return clientErrorResponse("sexo debe ser nulo cuando persona es nulo")
		}
		if newPersona.MesCierre != 0 {
			return clientErrorResponse("mesCierre debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.FechaNacimiento) > 0 {
			return clientErrorResponse("nacimiento debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.FechaFallecimiento) > 0 {
			return clientErrorResponse("fallecimiento debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.FechaInscripcion) > 0 {
			return clientErrorResponse("inscripcion debe ser nulo cuando persona es nulo")
		}
		if len(newPersona.FechaCierre) > 0 {
			return clientErrorResponse("fechaCierre debe ser nulo cuando persona es nulo")
		}
		if newPersona.NuevaCUIT != 0 {
			return clientErrorResponse("nuevaCuit debe ser nulo cuando persona es nulo")
		}
	...
	default:
		return clientErrorResponse("persona [" + newPersona.TipoPersona + "] invalido, debe ser F (Humana), J (Juridica) o nulo (para indicar que no se aplican cambios sobre el asset topo Persona) ")
	}
	if !personaNull {
		...
		case "I":
		default:
			return clientErrorResponse("estado [" + newPersona.EstadoCUIT + "] invalido, debe ser A (Activa) o I (Inactiva)")
		}
	}
