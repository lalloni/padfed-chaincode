...
func (s *SmartContract) queryPersona(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	case 2:
		...
		if err != nil {
			return s.peerResponse(clientErrorResponse("p_full [" + args[1] + "] invalido. " + err.Error()))
		}
	case 3:
		p_full, err = strconv.ParseBool(args[1])
		if err != nil {
		...
		if err != nil {
			return s.peerResponse(clientErrorResponse("p_composed [" + args[2] + "] invalido. " + err.Error()))
		}
	default:
		return s.peerResponse(clientErrorResponse("Número incorrecto de parámetros. Se esperaba {<CUIT>, [P_FULL], [P_COMPOSED]}"))
	}
